...
public abstract class AbstractDockerizer implements Component {
    ...
    public void removeAllContainersWithSameImage(){
        logger.debug("Removing system (imageName={})", imageName);
        try {
    ...
    public void startContainer() throws Exception {
        logger.debug(" Starting system (imageName={}, containerId={})", imageName, containerId);
    ...
    public void startMonitoringAndLogsReading(int since) throws Exception {
        logger.debug("Starting monitoring & logs reading for system (imageName={})", imageName);
...
//                                    logger.debug(line);
    ...
    public void waitForContainerFinish() throws DockerException, InterruptedException, DockerCertificateException {
        logger.debug("Waiting for system to finish (imageName={})", imageName);
        getDockerClient().waitContainer(containerId);

    }

    public void stopContainer(){
        logger.debug("Stopping system (imageName={})", imageName);
        try{
            if(containerId==null){
    ...
    public String createContainer() throws Exception {  // actual container creation

        if(findImagesByName(imageName).size()==0)
            prepareImage(imageName);

        logger.debug(" Creating system (imageName={})", imageName);
        ...
        logger.debug(" system create with (containerName:{})  and  (contId={})  ",containerName,contId  );
        if (contId == null) {
            IllegalStateException exception = new IllegalStateException(format("Unable to create container %s", containerName));
            logger.error(String.format("Failed to create system (imageName=%s): ",imageName), exception);
            throw exception;
        }
    ...
    private ContainerInfo getContainerInfo(DockerClient dockerClient) {
        ...
        try {
            ...
            logger.error("The system " + containerId + " is not known.", e);
        } catch (Throwable e) {
            logger.error("Error while checking status of the system " + containerId + ".", e);
        }
        return info;
    }
    ...
    private void connectContainerToNetworks(Collection<String> networks) throws DockerException, InterruptedException, DockerCertificateException {
        logger.debug("==============================  Connecting system to networks (imageName={})", imageName);
...
//        }
    ...
    private void removeAllSameNamesContainers() throws InterruptedException, DockerException, DockerCertificateException {
        logger.debug("Removing all systems with the same name ({})", containerName);
        for(Container container : findContainersByName(containerName, DockerClient.ListContainersParam.allContainers())) {
