...
public class ContainerCommandsReaction implements CommandReaction {
    ...
    private Map<String, Entry<String, String>> customContainersRunningWithSimpleAndOriginalIds = new HashMap<String, Entry<String, String>>();
    ...
    public void handleCmd(Byte command, byte[] bytes, String replyTo) throws Exception {
        ...
        if (command == Commands.DOCKER_CONTAINER_START){
            ...
            logger.debug("CONTAINER_START command received with imageName="+startCommandData.image+""); // sensemark/zookeeper

            String containerId=null;
            Component compToSubmit = null;
            String simpleContainerId = null;  //added

            String[] splitted = startCommandData.getImage().split("/");
            String cleanedImageName=splitted[splitted.length-1].split(":")[0];

            logger.debug("cleaned imageName=  "+cleanedImageName);
...
//                }
            ...
            if(systemAdapter !=null && startCommandData.image.equals(systemAdapterImageName)) {
                ...
                if(customContainers.containsKey(cleanedImageName)){    // as cleanedImageName was used in CommandReactionBuilder i.e.  customContainers.put(cleanedImageName, component)
                    /*
                ...
                if(AbstractDockerizer.class.isInstance(customComponent)){
                    ...
                    logger.debug("CCR %%%%%%%%%%%%%%%%  custom comp,dockerizer instance         compToSubmit " + compToSubmit +  "   containerId: "+ containerId);
                    ...
                    logger.debug("CCR %%%%%%%%%%%%%%%%  custom comp,java comp    compToSubmit " + compToSubmit +  "   containerId: "+ containerId);
                }
                ...
                */
                ...
                if(!imgName.contains(":"))
                    imgName+=":latest";
                logger.info("Trying to create system with imageName="+imgName);
            ...
            if( (compToSubmit!=null)  ){
                ...
                if(AbstractDockerizer.class.isInstance(compToSubmit)){
                    compToSubmit = ((AbstractDockerizer)compToSubmit).clone(new ArrayList(Arrays.asList(startCommandData.environmentVariables)));  // get relevant  dockerizer
                    containerId = ((AbstractDockerizer)compToSubmit).createContainerWithRemoveAllPrevs(startCommandData.environmentVariables);   // get docker container
                }else {
                    compToSubmit = compToSubmit.getClass().getConstructor().newInstance();
                    // added for fixing custom component duplicate creation

                }

                if(customContainers.containsKey(cleanedImageName)){
                    int runningCustomContainersCount = (customContainersRunning.containsKey(cleanedImageName)? customContainersRunning.get(cleanedImageName) :0);
                  //  containerId = cleanedImageName+"_"+runningCustomContainersCount;  // original cont id returned by docker client is
                     simpleContainerId = cleanedImageName+"_"+runningCustomContainersCount;
                    customContainersRunningWithSimpleAndOriginalIds.put(cleanedImageName, new AbstractMap.SimpleEntry(containerId,simpleContainerId ));
                    runningCustomContainersCount++;
                    customContainersRunning.put(cleanedImageName, runningCustomContainersCount);
                }


                componentsExecutor.submit(compToSubmit, containerId, startCommandData.getEnvironmentVariables()); // executes the init & run methods accordingly
